...
describe('compiler', () => {
    ...
    it('should transpile the blockquotes', () => {
        const markdown = '> This is a blockquote'
        const expectedHtml = '<blockquote>This is a blockquote</blockquote>'
        const actualHtml = transpile(markdown, true)
        expect(actualHtml).toBe(expectedHtml)
    })

    it('should transpile nested markdown inside block quotes', () => {
        const markdown = '> `Hello` __World__'
        const expectedHtml = '<blockquote><code>Hello</code><i>World</i></blockquote>'
        const actualHtml = transpile(markdown, true)
        expect(actualHtml).toBe(expectedHtml)
    })
})
